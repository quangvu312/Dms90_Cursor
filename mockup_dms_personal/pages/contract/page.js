  ...
  function createContractModule(moduleKey) {
    ...
    function uploadField(d, locked, fullWidth) {
      ...
      const list = files.length
        ? `<ul class="ct-file-list">${files.map((f, i) => `
            <li class="ct-file-list__item">
              <span class="ct-file-list__icon" aria-hidden="true">${ctIcon('file')}</span>
      ...
      const picker = locked ? '' : `<div class="ct-upload" data-action="ct-upload-pick">
          ...
          <span class="ct-upload__placeholder">Upload file hợp đồng (.pdf, .doc, .docx, .txt)</span>
    ...
    function onFilePicked(input) {
      ...
      if (invalid && !valid.length) {
        d._errors = Object.assign({}, d._errors, { file: 'Chỉ được upload file PDF, DOC, DOCX, TXT' });
        goMode(mode, d.id || '');
        return;
      }
      Promise.all(valid.map(toFileRecord)).then((recs) => {
        ...
        if (invalid) {
          d._errors = Object.assign({}, d._errors, { file: 'Một số file không đúng định dạng (PDF, DOC, DOCX, TXT) đã bị bỏ qua' });
