...
import {
  APP_CONTRACT_STATUSES,
  fmtDate,
  getTemplateContracts,
  getVisibleCustomerContracts,
  type AppContract,
} from '../contractData';
import { parseDmy } from '../../contract/store';

export type ContractTab = 'template' | 'customer';
...
function ContractCard({ item, tab }: { item: AppContract; tab: ContractTab }) {
  const navigate = useNavigate();
  return (
    <button
      type="button"
      onClick={() => navigate(`/sales-app/hop-dong/${item.id}?tab=${tab}`)}
    ...
    >
      <div className="min-w-0 flex-1">
        {item.module === 'template' ? (
          <p className="m-0 text-[11px] font-medium tracking-wide text-[#7587a6]">HỢP ĐỒNG MẪU</p>
        ) : null}
        <p className="m-0 mt-0.5 text-[14px] font-medium text-[#1f2937] leading-normal">{item.name}</p>
        <p className="m-0 mt-0.5 text-[14px]" style={{ color: SALES_APP_COLORS.boldBlue }}>{item.code}</p>
        {item.module === 'customer' && item.customerName ? (
...
export function AppContractListPage() {
  ...
  const tab: ContractTab = searchParams.get('tab') === 'customer' ? 'customer' : 'template';
  ...
  const [draft, setDraft] = useState({ customer: '', status: '', from: '', to: '' });
  ...
  const filterOn = !!(status || from || to || (tab === 'customer' && customer));

  const source = useMemo(
    () => (tab === 'template' ? getTemplateContracts() : getVisibleCustomerContracts()),
    [tab],
  );
  ...
  const rows = useMemo(() => {
    ...
    return source.filter((c) => {
      if (needle) {
        const hay = tab === 'template'
          ? `${c.code} ${c.name}`.toLowerCase()
          : `${c.code} ${c.name} ${c.customerCode || ''} ${c.customerName || ''}`.toLowerCase();
        if (!hay.includes(needle)) return false;
      }
      if (tab === 'customer' && customer && c.customerId !== customer && c.customerCode !== customer) return false;
  ...
  }, [q, customer, status, from, to, source, tab]);

  const emptySource = source.length === 0;
  const emptySearch = !emptySource && rows.length === 0;
      ...
      <div className="shrink-0 px-4 py-2 flex items-center gap-2 bg-[#f3f4f6]">
        <div className="flex-1 flex items-center gap-2 bg-white border border-slate-200 rounded-full px-3 min-h-11">
          ...
          <input
            ...
            placeholder={tab === 'template' ? 'Tìm mã HĐ, tên HĐ' : 'Tìm mã HĐ, tên HĐ hoặc khách hàng'}
        ...
        <button
          type="button"
          onClick={() => { setDraft({ customer, status, from, to }); setFilterOpen(true); }}
        ...
        {emptySource ? (
          <div className="pt-16 text-center">
            <p className="m-0 text-[15px] font-medium text-slate-800">
              {tab === 'template' ? 'Chưa có hợp đồng mẫu' : 'Chưa có hợp đồng khách hàng'}
            </p>
          </div>
        ) : emptySearch ? (
          <div className="pt-16 text-center">
            <p className="m-0 text-[15px] font-medium text-slate-800">Không tìm thấy hợp đồng phù hợp</p>
            {filterOn || q ? (
              <button
                type="button"
                className="mt-3 text-[#1437d6] text-[14px] bg-transparent border-0"
                onClick={() => { setQ(''); setCustomer(''); setStatus(''); setFrom(''); setTo(''); }}
              >
                Xóa bộ lọc
              </button>
            ) : null}
          </div>
        ) : rows.map((c) => <ContractCard key={c.id} item={c} tab={tab} />)}
      </div>
      ...
      {filterOpen ? (
        <div className="absolute inset-0 z-20 flex flex-col bg-white" role="dialog" aria-modal="true" aria-label="Bộ lọc">
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            <label className="block text-[13px] text-slate-600">Trạng thái
              <select className="mt-1 w-full min-h-11 rounded-lg border border-slate-200 px-3 text-[14px]" value={draft.status} onChange={(e) => setDraft((d) => ({ ...d, status: e.target.value }))}>
                <option value="">Tất cả</option>
                {APP_CONTRACT_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </label>
          ...
          <div className="shrink-0 p-4 border-t border-slate-100 flex gap-3">
            <button type="button" className="flex-1 min-h-12 rounded-lg border border-slate-200 text-[15px]" onClick={() => setDraft({ customer: '', status: '', from: '', to: '' })}>Đặt lại</button>
            <button
              type="button"
              className="flex-1 min-h-12 rounded-lg text-white text-[15px] font-medium"
              style={{ background: SALES_APP_COLORS.boldBlue }}
              onClick={() => { setCustomer(draft.customer); setStatus(draft.status); setFrom(draft.from); setTo(draft.to); setFilterOpen(false); }}
            >
              Áp dụng
            </button>
          </div>
