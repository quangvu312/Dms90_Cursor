...
(function () {
  ...
  function storyState() {
    if (!window.__tsStorySt) {
      window.__tsStorySt = {
        q: '', catalogId: '', channel: '',
        from: '', to: '', status: '', collapsed: true, page: 1, pageSize: 10
      };
    }
  ...
  function filterStories(list, st) {
    ...
    return (list || []).filter((s) => {
      ...
      if (st.status && S().storyStatus(s) !== st.status) return false;
  ...
  function storyColumns(offset) {
    ...
    return [
      ...
      { key: 'status', title: 'Trạng thái', render: (_, row) => DMS.render('StatusTag', { status: S().storyStatus(row) }) },
      { key: 'createdAt', title: 'Ngày tạo' },
  ...
  function composer(d, mode) {
    ...
    return `<div class="ts-composer">
      ...
      <aside class="ts-composer__side">
        <section class="ts-panel">
          <h3 class="ts-panel__title">Thông tin bài viết</h3>
          ...
          ${DMS.render('MultiSelect', { id: 'ts-promos', label: 'CTKM', values: d.promotionProgramIds || [], options: S().activePromoOptions(d.promotionProgramIds), placeholder: 'Chọn chương trình khuyến mãi...' })}
          ${DMS.render('MultiSelect', { id: 'ts-displays', label: 'CTTB', values: d.displayProgramIds || [], options: S().activeDisplayOptions(d.displayProgramIds), placeholder: 'Chọn chương trình trưng bày...' })}
        </section>
        <section class="ts-panel">
  ...
  function articleView(item) {
    ...
    const promoNames = (item.promotionProgramIds || []).map((id) => S().promoLabel(id)).filter(Boolean);
    const displayNames = (item.displayProgramIds || []).map((id) => S().displayLabel(id)).filter(Boolean);
    const meta = [
    ...
    return `<article class="ts-article">
      <p class="ts-article__kicker">${DMS.escape(S().catalogName(item.catalogId))}</p>
      <h1 class="ts-article__title">${DMS.escape(item.title)}</h1>
      <p class="ts-article__meta">${DMS.escape(meta)}${meta ? '  ·  ' : ''}${DMS.render('StatusTag', { status: S().storyStatus(item) })}</p>
      ${S().storyStatus(item) === 'Từ chối' && item.rejectionReason
        ? `<p class="ts-article__reject">Lý do từ chối: ${DMS.escape(item.rejectionReason)}</p>`
        : ''}
      ${cover}
      ...
      ${promoNames.length ? `<section class="ts-article__docs"><h2>Chương trình khuyến mãi</h2><ul class="ts-related">${promoNames.map((n) => `<li>${DMS.escape(n)}</li>`).join('')}</ul></section>` : ''}
      ${displayNames.length ? `<section class="ts-article__docs"><h2>Chương trình trưng bày</h2><ul class="ts-related">${displayNames.map((n) => `<li>${DMS.escape(n)}</li>`).join('')}</ul></section>` : ''}
  ...
  function readStoryDom(d) {
    ...
    d.promotionProgramIds = msValues('ts-promos');
    d.displayProgramIds = msValues('ts-displays');
  ...
  function saveStory() {
    ...
    const payload = {
      ...
      endDate: d.endDate,
      promotionProgramIds: d.promotionProgramIds || [],
      displayProgramIds: d.displayProgramIds || [],
    ...
    if (d.id) {
      ...
      if (item) {
        const prev = S().storyStatus(item);
        payload.status = prev || 'Khởi tạo';
  ...
  function onTsClick(e) {
    ...
    if (e.target.closest('[data-action="filter-reset"]')) {
      if (page.getAttribute('data-ts') === 'catalog') {
        ...
        window.__tsStorySt = { q: '', catalogId: '', channel: '', from: '', to: '', status: '', collapsed: true, page: 1, pageSize: ps };
        go('/telling-story/content', { mode: '', id: '' });
      }
      return;
    }
    ...
    if (a === 'ts-st-save') { saveStory(); return; }
