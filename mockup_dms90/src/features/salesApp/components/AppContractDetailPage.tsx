...
function parseBackTab(raw: string | null, kind: 'contract-type' | 'customer'): string {
  if (raw === 'customer') return 'customer';
  if (raw === 'template' || raw === 'contract-type') return 'contract-type';
  return kind === 'customer' ? 'customer' : 'contract-type';
}
...
export function AppContractDetailPage() {
  ...
  const found = findVisibleContract(id);
  const kind = found?.kind || (searchParams.get('tab') === 'customer' ? 'customer' : 'contract-type');
  const backTab = parseBackTab(searchParams.get('tab'), kind);
  const backTo = `/sales-app/hop-dong?tab=${backTab}`;
  const isType = found?.kind === 'contract-type';
  ...
  if (isType) {
    ...
    const rows: [string, string][] = [
      ['Mã loại hợp đồng', item.code],
      ['Tên loại hợp đồng', item.name],
      ['Đối tượng xem', item.viewerTypeLabel],
    ];
    if (item.viewerType && item.viewerValueFieldLabel) {
      rows.push([item.viewerValueFieldLabel, item.viewerValueLabel || '—']);
    }
    rows.push(['Ghi chú', item.note || '—']);

    return (
      <div className="flex-1 min-h-0 flex flex-col bg-[#f3f4f6]">
        <ScreenHeader title="Chi tiết loại hợp đồng" onBack={() => navigate(backTo)} />
          ...
          <article className="bg-white rounded-2xl shadow-[0_2px_3px_rgba(191,191,191,0.38)] px-3.5 py-3 mt-3">
            {rows.map(([label, value]) => (
              <div key={label} className="flex gap-3 py-2 border-b border-[#f1f5f9]">
                <dt className="w-[120px] shrink-0 text-[13px] text-[#7587a6] m-0">{label}</dt>
                <dd className="m-0 flex-1 text-[14px] text-[#1f2937] min-w-0 break-words">{value || '—'}</dd>
              </div>
            ))}
            <div className="flex gap-3 py-2">
              <dt className="w-[120px] shrink-0 text-[13px] text-[#7587a6] m-0">Trạng thái hoạt động</dt>
              <dd className="m-0">
                <span className={`inline-block px-2 py-0.5 rounded text-[11px] font-medium ${pillClass(item.status)}`}>{item.status}</span>
              </dd>
            </div>
          </article>
          <h2 className="text-[13px] font-medium text-[#7587a6] mt-4 mb-2">File hợp đồng mẫu</h2>
          <article className="bg-white rounded-2xl shadow-[0_2px_3px_rgba(191,191,191,0.38)] px-3.5 py-2">
            {item.templateFiles.length === 0 ? (
              <p className="text-[13px] text-[#6b7280]">Không có file đính kèm.</p>
            ) : item.templateFiles.map((f) => (
              <div key={f.id || f.name} className="flex items-center gap-2 py-2.5 border-b border-[#f1f5f9] last:border-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="m-0 text-[12px] text-[#6b7280]">{fileMetaLine(f)}</p>
                </div>
                <button
                  type="button"
