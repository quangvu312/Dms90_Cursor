...
import { activePromotionsForApp } from '../../tellingStory/programLinks';

export function AppPromotionListPage() {
  const navigate = useNavigate();
  const [q, setQ] = useState('');
  const all = useMemo(() => activePromotionsForApp(), []);
  const rows = useMemo(() => {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col bg-[#fafafa]">
      <ScreenHeader title="Chương trình khuyến mãi" onBack={() => navigate('/sales-app/khac')} />
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4 space-y-2.5 hide-scrollbar">
        {rows.length === 0 ? (
          <p className="text-center text-[13px] text-[#94a3b8] py-12 m-0">Không có chương trình khuyến mãi đang hoạt động</p>
        ) : (
          rows.map((p) => (
            <button
              key={p.id}
              type="button"
              onClick={() => navigate(`/sales-app/khuyen-mai/${p.id}`)}
            ...
            >
              <span className="min-w-0 flex-1">
                ...
                <p className="m-0 mt-1 text-[12px] text-[#6b7280]">{p.startAt || ''} → {p.endAt || ''}</p>
