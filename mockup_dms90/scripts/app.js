...
(function () {
  ...
  const COMPONENT_SCRIPTS = [
    ...
    'Tabs', 'Collapse', 'Menu', 'Sidebar', 'Header', 'Tree', 'TreeSelect', 'Tooltip', 'DonutChart', 'ActionIconButton'
  ];

  async function loadScript(src) {
    return new Promise((resolve, reject) => {
      const s = document.createElement('script');
      s.src = src + (src.includes('?') ? '&' : '?') + 'v=20260820-flt';
      s.onload = resolve;
      s.onerror = reject;
  ...
  async function fetchJSON(path) {
    const res = await fetch(path + (path.includes('?') ? '&' : '?') + 'v=20260820-flt');
