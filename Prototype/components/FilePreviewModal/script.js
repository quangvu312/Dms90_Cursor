/**
 * FilePreviewModal — xem trước file đính kèm (PDF / TXT / DOC / DOCX / JPG / PNG)
 */
(function (DMS) {
  'use strict';
  ...
  function previewBody(file) {
    ...
    if (kind === 'image') {
      ...
      return `<img class="ct-file-preview__img ts-file-preview__img" alt="${DMS.escape(file.name || 'Ảnh')}" src="${DMS.escape(src)}" />`;
    }
    if (kind === 'video') {
      const src = previewSrc(file);
