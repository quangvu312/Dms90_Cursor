(function (DMS) {
  ...
  DMS.register('TreeSelect', {
    ...
    bindAll(root) {
      ...
      if (!this._docBound) {
        ...
        document.addEventListener('click', (e) => {
          if (e.target.closest('.dms-treeselect, .dms-overlay-popup, #dms-overlay-root')) return;
    ...
    bind(el) {
      ...
      const dropdown = el.querySelector('.dms-treeselect__dropdown');
      const inHost = (sel) => el.querySelector(sel) || dropdown?.querySelector(sel);

      const setOpen = (open) => {
        el.classList.toggle('is-open', open);
        el.querySelector('.dms-treeselect__control')?.setAttribute('aria-expanded', open ? 'true' : 'false');
        const drop = DMS.getOverlay(el) || dropdown;
        if (open) {
          ...
          DMS.restoreOverlay(drop);
        }
      };

      const refreshTree = (keyword = '') => {
        const treeEl = inHost('[data-treeselect-tree]');
        if (!treeEl) return;
      ...
      const onTreeAction = (e) => {
        ...
        if (action === 'treeselect-open') {
          ...
          if (open) {
            refreshTree();
            inHost('.dms-treeselect__search')?.focus();
          }
          e.stopPropagation();
        }
        ...
        if (action === 'treeselect-toggle') {
          ...
          refreshTree(inHost('.dms-treeselect__search')?.value || '');
        }
      ...
      el.addEventListener('click', onTreeAction);
      dropdown?.addEventListener('click', onTreeAction);
