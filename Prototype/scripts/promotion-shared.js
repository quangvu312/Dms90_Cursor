  ...
  const PromoShared = {
    ...
    async loadStore() {
      if (!window.__promoStore) {
        window.__promoStore = await fetch('data/promotion.json?v=20260820-ts').then(r => r.json());
      }
    ...
    renderDetail(item) {
      ...
      const info = `<div class="promo-section"><div class="promo-section__head"><span class="promo-section__num">1</span> Thông tin chương trình</div>
        <dl class="promo-dl">
          ...
          ${item.storyId && window.TellingStoryShared ? `<dt>Tài liệu hướng dẫn</dt><dd>${TellingStoryShared.storyRefHtml(item.storyId)}</dd>` : ''}
        </dl></div>
