...
(function (global) {
  ...
  const ALLOWED_EXTS = ['pdf', 'doc', 'docx', 'txt', 'jpg', 'jpeg', 'png'];
  const ACCEPT_ATTR = '.pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,image/jpeg,image/png';
  const MIME = {
    pdf: 'application/pdf',
  ...
  const FORMAT_LABEL = 'PDF, DOC, DOCX, TXT, JPG, JPEG, PNG';
  const FORMAT_HINT = 'Hỗ trợ: PDF, DOC, DOCX, TXT, JPG, JPEG, PNG';
  const FORMAT_ERROR = 'Định dạng hỗ trợ: PDF, DOC, DOCX, TXT, JPG, JPEG, PNG.';
  const MODULES = {
    template: {
      key: 'template',
      label: 'Hợp đồng mẫu',
  ...
  function fileKind(file) {
    ...
    if (IMAGE_EXTS.indexOf(ext) !== -1 || IMAGE_MIMES.indexOf(type) !== -1) return 'image';
  ...
  function fileCategory(file) {
    return fileKind(file) === 'image' ? 'image' : 'document';
  }
  ...
  function filePreviewSrc(file) {
    return (file && (file.objectUrl || file.previewUrl || file.url)) || '';
  }
  function enrichFile(file, idHint) {
    if (!file || !file.name) return file;
    const ext = fileExt(file);
    if (!file.extension) file.extension = ext;
    if (!file.category) file.category = fileCategory(file);
    if (!file.type && MIME[ext]) file.type = MIME[ext];
    if (file.category === 'image' && !file.previewUrl) {
      file.previewUrl = file.objectUrl || file.url || '';
    }
    if (!file.id && idHint) file.id = idHint;
    return file;
  }
  ...
  async function loadStore() {
    ...
    const [contractRes, typeRes] = await Promise.all([
      fetch('data/contract.json?v=20260821-ctimg'),
      fetch('data/common-data-contract-types.json?v=20260821-ctimg')
    ]);
  ...
  function normalizeFiles(item) {
    if (!item) return [];
    let list = [];
    if (Array.isArray(item.files)) list = clone(item.files).filter((f) => f && f.name);
    else if (item.file && item.file.name) list = [clone(item.file)];
    const prefix = item.id || 'FILE';
    return list.map((f, i) => enrichFile(f, prefix + '-F' + String(i + 1).padStart(2, '0')));
  }
  ...
  global.ContractFile = {
    ALLOWED_EXTS, ACCEPT_ATTR, MIME, IMAGE_EXTS,
    FORMAT_LABEL, FORMAT_HINT, FORMAT_ERROR,
    ext: fileExt, kind: fileKind, category: fileCategory, isAllowed: isAllowedFile,
    typeLabel: fileTypeLabel, formatSize, previewSrc: filePreviewSrc, enrich: enrichFile
  };
