...
(function (global) {
  ...
  const DMS = {
    ...
    overlayHostSelector() {
      return '[data-select], [data-multiselect], [data-treeselect-id], [data-datepicker], .dms-datepicker, .dms-treeselect, .dms-action-dropdown';
    },

    overlayDropdownSelector() {
      return '.dms-select__dropdown, .dms-multiselect__dropdown, .dms-datepicker__dropdown, .dms-treeselect__dropdown, .dms-action-dropdown__menu';
    },
    ...
    placeOverlay(dropdown, anchor) {
      ...
      if (!dropdown.dataset.overlayClickBound) {
        ...
        if (!dropdown.classList.contains('dms-action-dropdown__menu')) {
          dropdown.addEventListener('click', (e) => e.stopPropagation());
        }
      }
      const rect = anchor.getBoundingClientRect();
      const isCalendar = dropdown.classList.contains('dms-datepicker__dropdown');
      const isActionMenu = dropdown.classList.contains('dms-action-dropdown__menu');
    ...
    closeAllOverlays(except) {
      document.querySelectorAll('.dms-select.is-open, .dms-multiselect.is-open, .dms-treeselect.is-open, .dms-datepicker.is-open, .dms-action-dropdown.is-open').forEach((el) => {
    ...
    bindFormControls(root) {
      ...
      this.get('RichTextEditor')?.bindAll?.(scope);
      if (!this._resetBound) {
