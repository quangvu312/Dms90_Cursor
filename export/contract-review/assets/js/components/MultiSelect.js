(function (DMS) {
  ...
  DMS.register('MultiSelect', {
    ...
    getValues(idOrEl) {
      ...
      if (!el) return [];
      return [...el.querySelectorAll('.dms-multiselect__list input:checked')].map(i => i.value);
    },
    ...
    bind(el) {
      ...
      const refreshTags = () => {
        ...
        const options = [...el.querySelectorAll('.dms-multiselect__list input')].map(i => ({
          value: i.value,
      ...
      const setOpen = (open) => {
        ...
        if (open) {
          ...
          el.querySelector('.dms-multiselect__search')?.focus();
        }
      };
      ...
      el.querySelector('[data-action="multiselect-search"]')?.addEventListener('input', (e) => {
        const kw = e.target.value.trim().toLowerCase();
        el.querySelectorAll('.dms-multiselect__option').forEach((opt) => {
