...
<https://app.visily.ai/projects/596df0d2-3f48-4378-85f5-feff1877965c/boards/1330515/elements/1067104185>

# 1 Config độ ưu tiên đối tượng áp dụng trong bảng giá mua

Cấu hình độ ưu tiên đối tượng áp dụng trong bảng giá : [PURCHASE\_PRICE\_PRIORITY\_RULE](https://kb.finviet.com.vn/pages/viewpage.action?pageId=53021846)

**Priority càng nhỏ → Độ ưu tiên càng cao**

|  |  |  |  |
| --- | --- | --- | --- |
| REGION | Vùng/Khu vực | Vùng/Khu vực | Priority = 3 |
| SALE\_CHANNEL | Kênh bán hàng | Kênh bán hàng | Priority = 2 |
| DISTRIBUTOR | Nhà phân phối | Nhà phân phối | Priority = 1 |
| .... |  |  |  |

# 2 Enhance bảng giá mua
...
* Tìm kiếm:
* + Theo Mã/Tên bảng giá, Vùng, Khu vực giống bên tab Danh sách bảng giá
  + Khi nhấn button này, hệ thống sẽ thực hiện lấy lại thông tin lần nữa và cập nhật danh sách bảng giá mới nhất theo đầy đủ các đối tượng áp dụng.

## 2.3 Chi tiết bảng giá
...
## 2.4 Tạo mới bảng giá

* Thay đổi giao diện thành 3 step để tạo mới bảng giá

### 2.4.1 Step Thông tin chung

* **Đến ngày**: Không bắt buộc nhập
* Các thông tin chung còn lại: Thông tin và quy tắc khai báo các trường dữ liệu giữ nguyên không thay đổi
* Button Tiếp tục:
...
| Điều kiện | Selectbox Onechoice | Có | Có | * Nếu có chọn điều kiện, thì mỗi điều kiện phải có ít nhất 1 dữ liệu được chọn * Khi chọn Thêm đối tượng; màn hình hiển thị Field "Đối tượng" * Placeholder: Chọn điều kiện * Danh sách điều kiện: Khi nhấp vào trường "Điều kiện", danh sách các tùy chọn sẽ xuất hiện, bao gồm:    + Vùng/Khu Vực   + Nhà phân phối * Mặc định khi mở màn hình không chọn đối tượng nào. |
...
#### 2.4.2.1 Đối tượng áp dụng = Vùng/Khu vực
...
* Dữ liệu Vùng/Khu vực được lấy từ màn hình Phân Vùng
  ...
  + **Dựa vào thông tin Vùng/Khu vực ở trường này để lấy thông tin bảng giá áp dụng cho NPP theo Vùng/Khu vực đã cài đặt**
...
#### 2.4.2.2 Đối tượng áp dụng = Kênh bán hàng

* Trường này cho phép người dùng chọn nhiều kênh bán hàng cùng lúc để cài đặt cho bảng giá.
* Dựa vào thông tin Kênh bán hàng ở danh sách này để lấy bảng giá áp dụng cho nhà phân phối theo kênh bán hàng đã cài đặt.
  + Trường hợp NPP không có kênh bán hàng thì không được áp dụng bảng giá cài đặt theo kênh bán hàng.
...
#### 2.4.2.3 Đối tượng áp dụng = Nhà phân phối
...
* Trường hợp tạo mới bảng giá: Không giới hạn danh sách sản phẩm được thêm vào, trường hợp chỉnh sửa bảng giá: Cho phép chọn tối đa 200 sản phẩm/1 lần thêm (Áp dụng cho thêm trên portal và thêm từ file import)
...
# 3 Quy tắc lấy giá mua

## 3.1 Quy tắc

Khi hệ thống cần xác định bảng giá áp dụng cho một nhà phân phối vào một ngày cụ thể, sẽ thực hiện theo các bước:

| Bước | Mô tã |
| --- | --- |
| Bước 1 | Xác định ngày tạo đơn hàng mua (PO) |
| Bước 2 | Xác định NPP thuộc đối tượng nào trong các đối tượng sau:   * Nhà phân phối * Vùng / Khu vực   → Lấy ra được tập dữ liệu của nhà phân phối, ví dụ:   * Nhà phân phối NPP1 * Khu vực HCM |
| Bước 3 | Xác định độ ưu tiên của các đối tượng ở bước 2 ở cấu hình : [PURCHASE\_PRICE\_PRIORITY\_RULE](https://kb.finviet.com.vn/pages/viewpage.action?pageId=53021846) |
| Bước 4 | * Lọc các bảng giá có Từ ngày trong Thời gian áp dụng <= Ngày tạo đơn hàng và Đến ngày >= Ngày tạo đơn hàng |
| Bước 5 | * Từ tập bảng giá ở bước 4 * Lấy ra các bảng giá có đối tượng áp dụng khớp với tập dữ liệu của nhà phân phối ở Bước 2. |
| Bước 6 | * Hệ thống bắt đầu từ đối tượng có độ ưu tiên **cao nhất (priority thấp nhất)** → đến thấp hơn theo config độ ưu tiên ở cấu hình : [PURCHASE\_PRICE\_PRIORITY\_RULE](https://kb.finviet.com.vn/pages/viewpage.action?pageId=53021846) * Kiểm tra các bảng giá theo từng cấp đối tượng như sau:    + Lọc bảng giá áp dụng cho đối tượng tương ứng      - Ưu tiên 1: bảng giá áp dụng theo *NPP*     - Ưu tiên 2: bảng giá theo kênh bán hàng     - Ưu tiên 3: bảng giá theo *Vùng / Khu vực*   + Tại mỗi cấp ưu tiên      - Kiểm tra xem nhà phân phối hiện tại có khớp với đối tượng của bảng giá hay không.     - Nếu có nhiều bảng giá cùng cấp → chọn bảng giá có `"Từ ngày"` lớn nhất nhưng **không vượt quá ngày đơn hàng**.   + Ngay khi tìm được bảng giá ở 1 cấp ưu tiên → DỪNG     - Không tiếp tục kiểm tra cấp ưu tiên thấp hơn nữa     - Tránh tình trạng chọn nhầm bảng giá rộng hơn   + Nếu xét hết các cấp ưu tiên mà không tìm thấy sẽ xét tới bảng giá base không có đối tượng áp dụng   + Nếu **không có bảng giá** → chuyển xuống cấp ưu tiên tiếp theo |
...
* Format tên file xuất ra: Danhsachbanggiamua\_DDMMYYYYHHMMSS
* File excel sẽ bao gồm 2 sheet:
  + Sheet Danh sách bảng giá: Chứa thông tin bảng giá, sản phẩm và giá
