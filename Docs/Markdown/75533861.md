...
* **Trạng thái gốc:** Trạng thái chính của đơn hàng trong hệ thống (ví dụ: Khởi tạo; Đã duyệt; Đã hủy; Đã xuất kho).
...
* **Trạng thái trung gian:** Một thông tin bổ sung, thể hiện trạng thái của một đối tượng (đơn hàng) bên trong một quy trình duyệt đang hoạt động. Ví dụ: Đang ở cấp 2, đợi "Admin HO" xử lý.

## Nội dung cập nhật

Trên lưới danh sách các đơn hàng của điểm bán thêm cột "**Trạng thái trung gian**" trên Màn hình **"[Đơn bán hàng](https://kb.finviet.com.vn/pages/viewpage.action?pageId=48444590&src=contextnavpagetreemode)**"; và màn hình [**"Tổng hợp đơn hàng điểm bán"**](https://kb.finviet.com.vn/pages/viewpage.action?pageId=53021728&src=contextnavpagetreemode) (2 màn hình hiển thị như nhau, chỉ khác nhau về phân quyền dữ liệu hiển thị nên mô tả về "Bộ lọc nâng cao và Trạng thái trung gian) bên dưới đây áp dụng cho cả 2 màn hình)

Màn hình:

#### **Trạng thái trung gian**

* Vị trí: Sau cột Trạng thái, trước cột Nhân viên
* Kiểu dữ liệu:  kiểu text
* Nội dung hiển thị:

  + Nếu một đơn hàng đang nằm trong một quy trình duyệt đang hoạt động, ô trong cột "Trạng thái trung gian" phải hiển thị một siêu liên kết (hyperlink) với dòng chữ "Xem chi tiết"
  + Nếu một đơn hàng không nằm trong bất kỳ quy trình duyệt nào đang hoạt động, ô trong cột "Trạng thái trung gian" sẽ được để trống
...
### Chi tiết bộ lọc nâng cao trên màn hình Đơn bán hàng và Tổng hợp đơn hàng điểm bán
...
2. **Cấu trúc Hiển thị Dữ liệu trong Popup:**
   ...
   * **Logic gom nhóm:** Dữ liệu phải được gom nhóm theo **"Mã - Tên quy trình"**. Mỗi nhóm quy trình sẽ hiển thị toàn bộ lịch sử các bước thuộc quy trình đó. Điều này giải quyết trường hợp một đơn hàng có thể áp dụng đồng thời nhiều quy trình (ví dụ: Quy trình Phê duyệt và Quy trình Hủy).
...
| **Trạng thái trung gian** | Data Column | Hiển thị trạng thái của điểm bán **sau khi** hành động được thực hiện.   * + Ví dụ 1: Đang ở cấp 2, đợi "ASM" xử lý.   + Ví dụ 2: Trạng thái trung gian = trạng thái gốc (khi quy trình kết thúc và hàm xử lý gốc được gọi để cập nhật trạng thái chính của đơn hàng). |
...
## **Quy trình và xử lý chung trên màn hình [Đơn bán hàng](https://kb.finviet.com.vn/pages/viewpage.action?pageId=48444590&src=contextnavpagetreemode)**
...
1. **Kiểm tra ban đầu**:
   ...
   2. **Logic:**Hệ thống phải kiểm tra xem 
      ...
      * Trạng thái chính của đơn hàng trong CSDL có còn là Trạng thái áp dụng hay không (ví dụ "Khởi tạo").
...
* + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt đơn hàng Điểm bán.  Vui lòng liên hệ quản trị viên."
  + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy đơn hàng Điểm bán.  Vui lòng liên hệ quản trị viên."
...
Một bản ghi (ví dụ: một đơn hàng điểm bán) đang ở trong một quy trình duyệt đang hoạt động. Người dùng hiện tại nhìn thấy bộ icon/ button xử lý của quy trình.
...
| 4.1 | Hệ thống | Không có quyền gốc => hiển thị thông báo.  Tắt popup và reload lại màn hình.  Đây là các lỗi xảy ra khi người dùng có quyền trong quy trình nhưng thiếu quyền gốc của màn hình.   * + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt đơn hàng Điểm bán.  Vui lòng liên hệ quản trị viên."   + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy đơn hàng Điểm bán.  Vui lòng liên hệ quản trị viên." |
| 5 | Hệ thống | **Có quyền gốc:**  **Thực thi Hành động Gốc:** Hệ thống gọi hàm xử lý gốc của màn hình   * Với core: Sell-out / Đơn hàng bán (NPP);  Sell-out - Duyệt/Hủy đơn hàng tại màn hình xem chi tiết * Với HT: [Luồng Indirect Sales](https://kb.finviet.com.vn/pages/viewpage.action?pageId=66523750#id-[HT]T%C3%A0ili%E1%BB%87ut%C3%ADchh%E1%BB%A3pERP-2.4.Lu%E1%BB%93ngIndirectSales)   **Cập nhật Trạng thái chính:** Hàm xử lý gốc sẽ thay đổi Trạng thái chính của bản ghi trong cơ sở dữ liệu (ví dụ: từ "Khởi tạo" sang "Đã duyệt" hoặc Từ “Khởi tạo" “sang "Đã hủy").  **Cập nhật Trạng thái trung gian:** Hệ thống cập nhật Trạng thái trung gian lần cuối để phản ánh hành động cuối cùng (Khi đó trạng thái trung gian bằng trạng thái chính của màn hình xử lý)  **Ghi Lịch sử:** Toàn bộ thông tin về hành động thay đổi trạng thái trung gian được ghi vào bảng lịch sử cập nhật trạng thái trung gian  **Kết quả:** Quy trình kết thúc thành công, bản ghi được cập nhật |

## **Các quy trình cài đặt trên màn hình:** **[Đơn bán hàng](https://kb.finviet.com.vn/pages/viewpage.action?pageId=48444590&src=contextnavpagetreemode)**
...
các button trên màn hình gồm:  và

### **1/ Quy trình xử lý Phê duyệt**

* **Mục đích:** Kiểm soát việc tạo mới và Phê duyệt một đơn hàng điểm bán, đảm bảo dữ liệu được xác thực qua nhiều cấp trước khi phê duyệt thành công các đơn hàng ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của đơn hàng điểm bán = Khởi tạo.
* **Hành động Gốc:** "Phê duyệt" sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả Sell-out / Đơn hàng bán (NPP)
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình Phê duyệt đơn hàng điểm bán | | | |
| Dữ liệu áp dụng | Duyệt Đơn hàng bán |  |  |  |
...
* **Kích hoạt:**Khi tạo mới một đơn hàng điểm bán, đơn hàng điểm bán đó có trạng thái Khởi tạo. Quy trình được kích hoạt khi Trạng thái chính = Trạng thái áp dụng

* **Thay đổi Giao diện:**
  + Trên dòng của đơn hàng đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình duyệt (đang hoạt động)**sẽ nhìn thấy icon xử lý "Duyệt" gốc của màn hình (logic xử lý: gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc của màn hình)
...
* + - Trạng thái trung gian mới nhất tại thời điểm hiện tại sẽ hiển thị khi hover chuột vào icon "Duyệt" mới của quy trình duyệt:
    - Button này chỉ hiển thị cho những người dùng thuộc Nhóm quyền được cấu hình để xử lý yêu cầu ở cấp hiện tại, sẽ thấy icon xử lý. Chọn button hiển thị popup:
      * + Trong đó: Xác nhận duyệt đơn hàng (NPP)
      ...
      * Chọn button Từ chối hiển thị popup bắt buộc nhập lý do:  
        + Nhập lý do dạng text. Chọn "Đồng ý" để xác nhận lý do và đóng popup "Xác nhận duyệt đơn hàng (NPP)"
...
* **Mục đích:** Cho phép người dùng có thẩm quyền Đã hủy một đơn hàng điểm bán đang ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của đơn hàng điểm bán = Khởi tạo.
* **Hành động Gốc:**  Đã hủy sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả Sell-out / Đơn hàng bán (NPP)
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình hủy đơn hàng điểm bán | | | |
| Dữ liệu áp dụng | Duyệt Đơn hàng bán |  |  |  |
| Trạng thái áp dụng | Khởi tạo |  |  |  |
| Duyệt vượt cấp | ON/ OFF |  |  |  |
| Chức năng | Hủy |  |  |  |
...
* Trên dòng của đơn hàng điểm bán đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình hủy đơn hàng điểm bán (đang hoạt động)**sẽ nhìn thấy icon xử lý "Hủy" gốc của màn hình (logic xử lý không thay đổi so với hiện tại)
* Đối với những người dùng **thuộc các Nhóm quyền trong quy trình duyệt (đang hoạt động) và thuộc cấp chờ xử lý của quy trình** sẽ nhìn thấy icon xử lý "Hủy"
  ...
  + Button này chỉ hiển thị cho những người dùng thuộc Nhóm quyền được cấu hình để xử lý yêu cầu ở cấp hiện tại, sẽ thấy icon xử lý. Chọn button hiển thị popup:
    - * Trong đó: Label: Xác nhận hủy đơn hàng bán
      * @trạng thái trung gian: Là dòng trạng thái trung gian hiển thị mới nhất theo thời điểm hover chuột
      * @Hành động khi Duyệt: Hiển thị hành động khi duyệt của quy trình theo cấp đang xử lý
