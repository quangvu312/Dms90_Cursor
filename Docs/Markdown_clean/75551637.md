...
| Epic |  |
| Feature |  |
| Description | Cung cấp một cửa sổ xem chi tiết, minh bạch về toàn bộ quá trình duyệt của một bản ghi cụ thể. Có thể xem Chi tiết lịch sử cập nhật trạng thái trung gian trên cả 2 màn hình Kiểm kho (NPP) và Duyệt kiểm kho NPP |
...
* **Trạng thái gốc:** Trạng thái chính của phiếu kiểm kho trong hệ thống (ví dụ: Khởi tạo; Đã duyệt; Đã hủy).
...
* **Trạng thái trung gian:** Một thông tin bổ sung, thể hiện trạng thái của một đối tượng (phiếu kiểm kho) bên trong một quy trình duyệt đang hoạt động. Ví dụ: Đang ở cấp 2, đợi "Admin HO" xử lý.

## Nội dung cập nhật

Trên lưới danh sách các phiếu kiểm kho thêm cột "**Trạng thái trung gian**" trên 

### Màn hình **Kiểm kho**

Trạng thái trung gian trên màn hình Kiểm kho:

* Vị trí: Sau cột Trạng thái, trước cột Lý do
* Kiểu dữ liệu:  kiểu text
* Nội dung hiển thị:

  + Nếu một phiếu kiểm kho đang nằm trong một quy trình duyệt đang hoạt động, ô trong cột "Trạng thái trung gian" phải hiển thị một siêu liên kết (hyperlink) với dòng chữ "Xem chi tiết"
  + Nếu một phiếu kiểm kho không nằm trong bất kỳ quy trình duyệt nào đang hoạt động, ô trong cột "Trạng thái trung gian" sẽ được để trống
...
* Lý do: Hiển thị  lý do khi chuyển trạng thái gốc của bản ghi

### Màn hình **Duyệt kiểm kho NPP**

Trạng thái trung gian trên màn hình Duyệt kểm kho NPP:
...
2. **Cấu trúc Hiển thị Dữ liệu trong Popup:**
   ...
   * **Logic gom nhóm:** Dữ liệu phải được gom nhóm theo **"Mã - Tên quy trình"**. Mỗi nhóm quy trình sẽ hiển thị toàn bộ lịch sử các bước thuộc quy trình đó. Điều này giải quyết trường hợp một phiếu kiểm kho có thể áp dụng đồng thời nhiều quy trình (ví dụ: Quy trình Phê duyệt và Quy trình Hủy).
...
| **Trạng thái trung gian** | Data Column | Hiển thị trạng thái của Phiếu kiểm kho **sau khi** hành động được thực hiện.   * + Ví dụ 1: Đang ở cấp 2, đợi "Admin 2" xử lý.   + Ví dụ 2: Trạng thái trung gian = trạng thái gốc (khi quy trình kết thúc và hàm xử lý gốc được gọi để cập nhật trạng thái chính của phiếu kiểm kho). |
...
1. **Kiểm tra ban đầu**:
   ...
   2. **Logic:**Hệ thống phải kiểm tra xem 
      ...
      * Trạng thái chính của phiếu kiểm kho trong CSDL có còn là Trạng thái áp dụng hay không (ví dụ "Khởi tạo").
...
* + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt Phiếu kiểm kho. Vui lòng liên hệ quản trị viên."
  + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy Phiếu kiểm kho. Vui lòng liên hệ quản trị viên."
...
Một bản ghi (ví dụ: một Phiếu kiểm kho của Nhà phân phối) đang ở trong một quy trình duyệt đang hoạt động. Người dùng hiện tại nhìn thấy bộ icon/ button xử lý của quy trình.
...
| 4.1 | Hệ thống | Không có quyền gốc => hiển thị thông báo.  Tắt popup và reload lại màn hình.  Đây là các lỗi xảy ra khi người dùng có quyền trong quy trình nhưng thiếu quyền gốc của màn hình.   * + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt Phiếu kiểm kho. Vui lòng liên hệ quản trị viên."   + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy Phiếu kiểm kho. Vui lòng liên hệ quản trị viên." |
...
## **Các quy trình cài đặt trên màn hình Kiểm kho**
...
* **Mục đích:** Kiểm soát việc tạo mới và Phê duyệt một Phiếu kiểm kho của Nhà phân phối, đảm bảo dữ liệu được xác thực qua nhiều cấp trước khi phê duyệt thành công các phiếu kiểm kho ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của Phiếu kiểm kho của Nhà phân phối = Khởi tạo.
* **Hành động Gốc:** "Phê duyệt" sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình Phê duyệt Phiếu kiểm kho của Nhà phân phối | | | |
| Dữ liệu áp dụng | Duyệt kiểm kho nhà phân phối |  |  |  |
| Trạng thái áp dụng | Khởi tạo |  |  |  |
| Duyệt vượt cấp | ON/ OFF |  |  |  |
| Chức năng | Phê duyệt |  |  |  |
| Trạng thái sau áp dụng | Chờ duyệt |  |  |  |
| **Cấp duyệt** | **Nhóm quyền** | ***Hành động khi Duyệt*** | **Hành động khi Từ chối** | **Tự động duyệt (ngày)** |
| Cấp 1 | Admin NPP 1 | *Đi tới cấp tiếp theo* | Thay đổi sang trạng thái Đã hủy | 2 |
| Cấp 2 | Admin NPP 2 | *Đi tới cấp tiếp theo* | Thay đổi sang trạng thái Đã hủy | 0 |
| Cấp ... | ... | *Đi tới cấp tiếp theo* | *Trở về cấp duyệt trước*hoặc *Thay đổi sang trạng thái Đã hủy* | 0 |
| Cấp 10 | Admin NPP 3 | *Thay đổi sang trạng thái **Chờ duyệt*** | *Trở về cấp duyệt trước* | 3 |

#### **Mô tả UI hiển thị:**

* **Kích hoạt:**Khi tạo mới một Phiếu kiểm kho của Nhà phân phối, Phiếu kiểm kho của Nhà phân phối đó có trạng thái Khởi tạo. Quy trình được kích hoạt khi Trạng thái chính = Trạng thái áp dụng

* **Thay đổi Giao diện:**
  + Trên dòng của phiếu kiểm kho đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình duyệt (đang hoạt động)**sẽ nhìn thấy icon xử lý "Duyệt" gốc của màn hình (logic xử lý: gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc của màn hình)
...
* + - Trạng thái trung gian mới nhất tại thời điểm hiện tại sẽ hiển thị khi hover chuột vào icon "Duyệt" mới của quy trình duyệt:
    - Button này chỉ hiển thị cho những người dùng thuộc Nhóm quyền được cấu hình để xử lý yêu cầu ở cấp hiện tại, sẽ thấy icon xử lý. Chọn button hiển thị popup:
      * + Trong đó: Xác nhận duyệt phiếu kiểm kho
      ...
      * Chọn button Từ chối hiển thị popup bắt buộc nhập lý do:  
        + Nhập lý do dạng text. Chọn "Đồng ý" để xác nhận lý do và đóng popup "Xác nhận duyệt phiếu kiểm kho"
        + Chọn Hủy để đóng popup "Xác nhận duyệt phiếu kiểm kho"
...
* **Mục đích:** Cho phép người dùng có thẩm quyền Đã hủy một Phiếu kiểm kho của Nhà phân phối đang ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của Phiếu kiểm kho của Nhà phân phối = Khởi tạo.
* **Hành động Gốc:**  Đã hủy sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình hủy Phiếu kiểm kho của Nhà phân phối | | | |
| Dữ liệu áp dụng | Duyệt kiểm kho nhà phân phối |  |  |  |
| Trạng thái áp dụng | Khởi tạo |  |  |  |
| Duyệt vượt cấp | ON/ OFF |  |  |  |
| Chức năng | Hủy |  |  |  |
...
| **Cấp duyệt** | **Nhóm quyền** | ***Hành động khi duyệt*** | **Hành động khi Từ chối** | **Tự động duyệt (ngày)** |
| Cấp 1 | Admin NPP 1 | *Đi tới cấp tiếp theo* | Không thay đổi | 2 |
| Cấp 2 | Admin NPP 2 | *Đi tới cấp tiếp theo* | *Trở về cấp duyệt trước* | 0 |
| Cấp ... | ... | *Đi tới cấp tiếp theo* | *Trở về cấp duyệt trước* | 0 |
| Cấp 10 | Admin NPP 3 | *Thay đổi sang trạng thái Đã hủy* | *Trở về cấp duyệt trước* | 3 |
...
* Trên dòng của Phiếu kiểm kho của Nhà phân phối đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình hủy Phiếu kiểm kho của Nhà phân phối (đang hoạt động)**sẽ nhìn thấy icon xử lý "Hủy" gốc của màn hình (logic xử lý không thay đổi so với hiện tại)
* Đối với những người dùng **thuộc các Nhóm quyền trong quy trình duyệt (đang hoạt động) và thuộc cấp chờ xử lý của quy trình** sẽ nhìn thấy icon xử lý "Hủy"
  ...
  + Button này chỉ hiển thị cho những người dùng thuộc Nhóm quyền được cấu hình để xử lý yêu cầu ở cấp hiện tại, sẽ thấy icon xử lý. Chọn button hiển thị popup:
    - * Trong đó: Label: Xác nhận hủy phiếu kiểm kho
      * @trạng thái trung gian: Là dòng trạng thái trung gian hiển thị mới nhất theo thời điểm hover chuột
      * @Hành động khi Duyệt: Hiển thị hành động khi duyệt của quy trình theo cấp đang xử lý
...
## **Các quy trình cài đặt trên màn hình Duyệt kiểm kho NPP**
...
* **Mục đích:** Kiểm soát việc tạo mới và Phê duyệt một Phiếu kiểm kho của Nhà phân phối, đảm bảo dữ liệu được xác thực qua nhiều cấp trước khi phê duyệt thành công các phiếu kiểm kho ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của Phiếu kiểm kho của Nhà phân phối = Chờ duyệt
* **Hành động Gốc:** "Phê duyệt" sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả
* **Trạng thái sau áp dụng:** Hoàn thành (nếu duyệt thành công)
* **Trạng thái khi Từ chối thành công:** Đã từ chối (nếu Hủy/ từ chối thành công).
...
| Duyệt vượt cấp | ON/ OFF |  |  |  |
| Chức năng | Phê duyệt |  |  |  |
| Trạng thái sau áp dụng | Hoàn thành |  |  |  |
| **Cấp duyệt** | **Nhóm quyền** | ***Hành động khi Duyệt*** | **Hành động khi Từ chối** | **Tự động duyệt (ngày)** |
| Cấp 1 | Admin NPP 1 | *Đi tới cấp tiếp theo* | Thay đổi sang trạng thái Đã từ chối | 2 |
| Cấp 2 | Admin NPP 2 | *Đi tới cấp tiếp theo* | Thay đổi sang trạng thái Đã từ chối | 0 |
| Cấp ... | ... | *Đi tới cấp tiếp theo* | *Trở về cấp duyệt trước*hoặc *Thay đổi sang trạng thái Đã từ chối* | 0 |
| Cấp 10 | Admin NPP 3 | *Thay đổi sang trạng thái **Hoàn thành*** | *Trở về cấp duyệt trước* | 3 |

#### **Mô tả UI hiển thị:**

* **Kích hoạt:**Khi tạo mới một Phiếu kiểm kho của Nhà phân phối, Phiếu kiểm kho của Nhà phân phối đó có trạng thái Chờ duyệt. Quy trình được kích hoạt khi Trạng thái chính = Trạng thái áp dụng
...
* **Mục đích:** Cho phép người dùng có thẩm quyền để Từ chối duyệt một Phiếu kiểm kho của Nhà phân phối đang ở trạng thái Chờ duyệt.
* **Điều kiện kích hoạt:** Trạng thái gốc của Phiếu kiểm kho của Nhà phân phối = Chờ duyệt.
* **Hành động Gốc:**  Đã từ chối sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả
* **Trạng thái sau áp dụng:** Đã từ chối (nếu đồng ý Đã hủy qua n cấp thành công)
* Lưu ý: "Từ chối/ hủy " trong trường hợp này thực chất là một hành động Phê duyệt cho việc từ chối/Hủy.
...
* Trên dòng của Phiếu kiểm kho của Nhà phân phối đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình hủy Phiếu kiểm kho của Nhà phân phối (đang hoạt động)**sẽ nhìn thấy icon xử lý "Hủy" gốc của màn hình (logic xử lý không thay đổi)
* Đối với những người dùng **thuộc các Nhóm quyền trong quy trình duyệt (đang hoạt động) và thuộc cấp chờ xử lý của quy trình** sẽ nhìn thấy icon xử lý "Hủy"
  + Trạng thái trung gian mới nhất tại thời điểm hiện tại sẽ hiển thị khi hover chuột vào icon "Hủy" mới của quy trình duyệt:
  + Button này chỉ hiển thị cho những người dùng thuộc Nhóm quyền được cấu hình để xử lý yêu cầu ở cấp hiện tại, sẽ thấy icon xử lý. Chọn button hiển thị popup:
    - * Trong đó: Label: Xác nhận từ chối phiếu kiểm kho
      * @trạng thái trung gian: Là dòng trạng thái trung gian hiển thị mới nhất theo thời điểm hover chuột
      * @Hành động khi Duyệt: Hiển thị hành động khi duyệt của quy trình theo cấp đang xử lý
      * @Hành động khi Từ chối: Hiển thị hành động khi từ chối của quy trình theo cấp đang xử lý
      * Nhập lý do:  free text
