...
* **Trạng thái gốc:** Trạng thái chính của đơn hàng trong hệ thống (ví dụ: Khởi tạo; Đã duyệt; Đã hủy).
...
Trên lưới danh sách các đơn hàng thêm cột "**Trạng thái trung gian**" trên Màn hình **[Bảng Giá Bán](https://kb.finviet.com.vn/pages/viewpage.action?pageId=48439313)**
...
| **Trạng thái trung gian** | Data Column | Hiển thị trạng thái của Bảng giá **sau khi** hành động được thực hiện.   * + Ví dụ 1: Đang ở cấp 2, đợi "Admin 2" xử lý.   + Ví dụ 2: Trạng thái trung gian = trạng thái gốc (khi quy trình kết thúc và hàm xử lý gốc được gọi để cập nhật trạng thái chính của đơn hàng). |
...
* + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt Bảng giá bán.  Vui lòng liên hệ quản trị viên."
  + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**
    - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy Bảng giá bán.  Vui lòng liên hệ quản trị viên."
...
Một bản ghi (ví dụ: một Bảng giá bán) đang ở trong một quy trình duyệt đang hoạt động. Người dùng hiện tại nhìn thấy bộ icon/ button xử lý của quy trình.
...
| 4.1 | Hệ thống | Không có quyền gốc => hiển thị thông báo.  Tắt popup và reload lại màn hình.  Đây là các lỗi xảy ra khi người dùng có quyền trong quy trình nhưng thiếu quyền gốc của màn hình.   * + **Tình huống A: Thiếu quyền ở bước cuối cùng (Quyền Phê duyệt)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Phê duyệt Bảng giá bán. Vui lòng liên hệ quản trị viên."   + **Tình huống B: Thiếu quyền khi Từ chối (Quyền Hủy)**     - **Hiển thị thông báo:**"Bạn có quyền xử lý trong quy trình này, nhưng không có quyền gốc để Hủy Bảng giá bán. Vui lòng liên hệ quản trị viên." |
...
* **Mục đích:** Kiểm soát việc tạo mới và Phê duyệt một Bảng giá bán, đảm bảo dữ liệu được xác thực qua nhiều cấp trước khi phê duyệt thành công các đơn hàng ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của Bảng giá bán = Khởi tạo.
* **Hành động Gốc:** "Phê duyệt" sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả [HO] Quản lý bảng giá bán
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình Phê duyệt Bảng giá bán | | | |
| Dữ liệu áp dụng | Duyệt Bảng giá bán |  |  |  |
...
* **Kích hoạt:**Khi tạo mới một Bảng giá bán, Bảng giá bán đó có trạng thái Khởi tạo. Quy trình được kích hoạt khi Trạng thái chính = Trạng thái áp dụng
...
* **Mục đích:** Cho phép người dùng có thẩm quyền Đã hủy một Bảng giá bán đang ở trạng thái Khởi tạo.
* **Điều kiện kích hoạt:** Trạng thái gốc của Bảng giá bán = Khởi tạo.
* **Hành động Gốc:**  Đã hủy sẽ gọi hàm xử lý gốc của màn hình để xử lý chuyển Trạng thái gốc theo mô tả [HO] Quản lý bảng giá bán
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Quy trình hủy Bảng giá bán | | | |
| Dữ liệu áp dụng | Duyệt bảng giá bán |  |  |  |
| Trạng thái áp dụng | Khởi tạo |  |  |  |
| Duyệt vượt cấp | ON/ OFF |  |  |  |
| Chức năng | Hủy |  |  |  |
...
* Trên dòng của Bảng giá bán đó, những người dùng KHÔNG **thuộc các Nhóm quyền trong quy trình hủy Bảng giá bán (đang hoạt động)**sẽ nhìn thấy icon xử lý "Hủy" gốc của màn hình (logic xử lý không thay đổi so với hiện tại)
