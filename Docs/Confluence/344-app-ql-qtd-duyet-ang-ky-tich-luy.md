...
## Màn hình: Xét duyệt đăng ký CTTL > Danh sách đăng ký

Người dùng login vào app QL và mở chức năng Xét duyệt đăng ký CTTL:

* Khác → Xét duyệt đăng ký CTTL > **Danh sách đăng ký → Tab Chờ duyệt**
* Hoặc Khác → Xét duyệt đăng ký CTTL > Danh sách đăng ký → Tab Chờ duyệt → Chọn xem **Chi tiết đăng ký** có trạng thái "Chờ duyệt"
      ...
      * Các trường hợp ẩn/ hiển thị button
        ...
        **Case D: List Điểm bán KHÔNG trong quy trình nào**
        ...
        + - **Hiển thị:** luôn hiển thị các button xử lý gốc của màn hình.
          - link: [Manager\_App] Xét duyệt đăng ký CTTL
...
Trạng thái chính của Phiếu đăng ký chương trình của điểm bán được chuyển từ Chờ duyệt sang Đã duyệt hoặc Chờ duyệt sang Từ chối duyệt, khi reload màn hình sẽ chuyển qua các Tab tương ứng, khi đó các bản ghi thuộc các Tab "Đã duyệt", "Từ chối" không thay đổi (theo link:[Manager\_App] Xét duyệt đăng ký CTTL)
...
2. **Trạng thái trung gian:** Trạng thái của Phiếu đăng ký chương trình của điểm bán bên trong một [quy trình duyệt](https://kb.finviet.com.vn/pages/viewpage.action?pageId=75536315), thể hiện cấp đang chờ xử lý.
3. **[Quyền Lớp 1 (Quyền Quy trình)](https://kb.finviet.com.vn/pages/viewpage.action?pageId=73143596):** Quyền của người dùng được tham gia vào một cấp duyệt trong quy trình duyệt [HO] Quy trình duyệt / [HO] Quy trình duyệt - Đăng ký tích lũy
4. **Quyền Lớp 2 (**Quyền xử lý gốc**):** Quyền cơ bản của người dùng để thực thi thành công hành động cuối cùng làm thay đổi trạng thái gốc trên app QL [Manager\_App] Xét duyệt đăng ký CTTL
...
* **Câu hỏi:** "Bản ghi này có đang trong một quy trình duyệt nào đang hoạt động không?" [HO] Quy trình duyệt - Đăng ký tích lũy
* **Nếu CÓ:**
  ...
  + **Kiểm tra Quyền Lớp 1:** "Người dùng userId có thuộc Nhóm quyền đang chờ xử lý ở cấp hiện tại không?"
    ...
    - Hiển thị ưu tiên trước rồi mới đến các lỗi khác của quy trình duyệt ( [Manager\_App] Xét duyệt đăng ký CTTL) .Tắt popup và reload lại màn hình khi đó button xử lý của người dùng sẽ ẩn trên các bản ghi. Quy trình dừng lại.
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| Tên quy trình | Phê duyệt Phiếu đăng ký chương trình của điểm bán | | | |
...
* Chọn "Trả lại/Kết thúc" để Trả lại yêu cầu duyệt→ khi đó hiển thị popup: 
  ...
  + Danh sách lý do lấy từ [Danh sách dữ liệu chung](https://kb.finviet.com.vn/pages/viewpage.action?pageId=53023360) loại = Lý do từ chối duyệt CTTL + thêm Lý do **Khác**, trường hợp user chọn Lý do khác phải nhập thông tin Lý do (free text, 100). Nếu không nhập lý do hoặc không chọn bất kỳ option nào thì Disable button
  + Nhấn xác nhận
...
2. **BE (Backend) xử lý từng Phiếu đăng ký chương trình của điểm bán:**
   ...
   * **Vòng lặp xử lý: theo link: [Manager\_App] Xét duyệt đăng ký CTTL**
...
* + Popup báo cáo tổng hợp (ưu tiên cao nhất trong list các popup báo lỗi hoặc gọp tất cả trên một popup lỗi): 
    ...
    - **Popup hiển thị gồm:**
      * "Phiếu đăng ký chương trình của điểm bán @Mã ĐB - Tên ĐB 1, @Mã ĐB - Tên ĐB n đã được cập nhật bởi một người dùng khác"
      * "Bạn không có quyền xử lý các Phiếu đăng ký chương trình của điểm bán: @Mã ĐB - Tên ĐB 1, @Mã ĐB - Tên ĐB n"
      * Lỗi khác theo logic xử lý tại link: [Manager\_App] Xét duyệt đăng ký CTTL
